...
本文档归纳博客双仓库项目在优化、E2E 与 **Jekyll → Astro 迁移（2026-05）** 中的问题及处理方式。架构见 [ARCHITECTURE.md](ARCHITECTURE.md)。依赖安全见 [DEPENDENCIES.md](DEPENDENCIES.md)。
...
- 合并前：`npm run verify:local`（含 audit 与 VPS 子路径断言）。`run-ci-parity.sh` 仅为 CI 子集，不是替代。
