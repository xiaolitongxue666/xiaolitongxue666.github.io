...
// pagination-page 由 FloatingButtons.astro 注入；须与 src/lib/pagination.ts 的 POSTS_PER_PAGE 一致
(function () {
  'use strict';

  function getPaginationPage() {
    const paginationMeta = document.querySelector(
      'meta[name="pagination-page"]',
    );
    if (paginationMeta) {
  ...
  function goToPagination() {
    ...
    const paginationUrl =
      pageNumber === 1 ? siteBase : `${siteBase}page${pageNumber}/`;
